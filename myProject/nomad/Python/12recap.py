...
if response.status_code != 200: 
    #response.status_code로 status_code 확인 만약 status_code 가 200이라면 정상적인 상태
    print("Can't request website")
else: 
    results = []
   # print(response.text, "html.parser") #response로 하면 숫자값 text로 하면 구성하고 있는 전체 html코드를 확인 할 수 있음 
    soup = BeautifulSoup(response.text, "html.parser") #text를 soup쪽에 넘겨줌 전체 페이지라는 의미로 html.parser
     
    jobs =soup.find_all('section', class_='jobs') #soup에서 담고있는 전체 html문서에서 section tag의 class부분만 넘김 
    #class_="jobs" argument의 이름을 넣어주면 argument 의 순서를 상관없게 해줌 원래는 순서대로 들어가야 함 
    ...
    for job_section in jobs:
        ...
        for post in job_posts:
            ...
            anchor = anchors[1] #BeautifulSoup는 딕셔너리 처럼 바꿔줌
            link = anchor['href']
